...
import com.acerchem.order.data.CardTypeDataList;
import com.acerchem.storesession.data.CurrencyDataList;
import com.acerchem.storesession.data.LanguageDataList;
import com.acerchem.user.data.CountryDataList;
import com.acerchem.user.data.TitleDataList;
...
public class MiscsController extends BaseController
{
	...
	@RequestMapping(value = "/{baseSiteId}/deliveryRegions", method = RequestMethod.GET)
	@ResponseBody
	public RegionListWsDTO getDeliveryRegions(@RequestParam("countryIsoCode") final String countryIsoCode,@RequestParam(defaultValue = DEFAULT_FIELD_SET) final String fields)
	{
		List<RegionData> Regions=i18NFacade.getRegionsForCountryIso(countryIsoCode);
		final RegionListWsDTO dataList = new RegionListWsDTO();
		dataList.setRegions(Regions);
		return dataList;
	}
