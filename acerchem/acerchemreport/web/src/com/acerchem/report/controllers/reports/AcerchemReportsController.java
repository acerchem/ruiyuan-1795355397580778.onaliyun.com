package com.acerchem.report.controllers.reports;
...
import com.acerchem.report.data.AcerchemCategoryBean;
import com.acerchem.report.data.AcerchemEmployeeSalesBean;
import com.acerchem.report.data.CustomerCreditAnalysisForReportBean;
import com.acerchem.report.data.CustomerSalesAnalysisForm;
import com.acerchem.report.data.MonthlySalesAnalysisForm;
import com.acerchem.report.data.ProductSalesForm;
import com.acerchem.report.data.SearchCriteriaFrom;
import com.acerchem.report.data.VendorAnalysisForm;
import com.acerchem.report.data.VendorInventoryForm;


@Controller
@RequestMapping("/reports")
public class AcerchemReportsController extends AbstractSearchPageController
{// AbstractPageController
			...
			@RequestParam(value = "show", defaultValue = "Page") final ShowMode showMode) throws CMSItemNotFoundException
	{
		...
		final List<OrderDetailsReportData> searchPageData = acerchemOrderDao.getOrderDetails(curMonth, searchCriteriaFrom.getArea(),
				searchCriteriaFrom.getCountryCode(), searchCriteriaFrom.getUserName(), searchCriteriaFrom.getOrderCode());
	...
	public String getMonthlySalesAnalysisPage(final MonthlySalesAnalysisForm monthlySalesAnalysisForm, final Model model)
	...
	public String getVendorOrderProductTemp(final Model model, final VendorAnalysisForm form) throws ParseException
	{
		...
		final List<OrderProductReportData> list = acerChemProductService.getOrderProductByVendor(uid, start, end);
		model.addAttribute("list", list);
		return "pages/reports/vendorOrderProduct";
	}
			...
			throws CMSItemNotFoundException
	{
		...
		final List<CustomerSalesAnalysisData> list = acerchemOrderAnalysisService.getCustomerSalesAnalysis(area, name, amount);
			...
			@RequestParam("endDate") final String endDate) throws CMSItemNotFoundException, ParseException
	{
		...
		if (list.size() > 0)
		{
			...
			for (final CustomerBillAnalysisData data : list)
			{
				...
				{
					outerNinetyPayAmount = CommonConvertTools.addDouble(outerNinetyPayAmount, data.getOuterNinetyPayAmount());
