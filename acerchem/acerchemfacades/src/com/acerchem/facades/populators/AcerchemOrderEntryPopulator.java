...
public class AcerchemOrderEntryPopulator implements Populator<AbstractOrderEntryModel, OrderEntryData> {
	...
	public void populate(AbstractOrderEntryModel source, OrderEntryData target) {
			...
			if (CollectionUtils.isNotEmpty(source.getConsignmentEntries())) {
				...
				List<ConsignmentEntryModel> entries = new ArrayList<ConsignmentEntryModel>(
						source.getConsignmentEntries());
				// List<ConsignmentEntryData> dataEntris =
				// getConsignmentEntryConverter().convertAll(entries);
	...
	protected void addConfigurations(final AbstractOrderEntryModel source, final OrderEntryData target) {
		final List<ConfigurationInfoData> configurations = orderEntryProductInfoConverter
				.convertAll(source.getProductInfos()).stream().flatMap(Collection::stream).collect(Collectors.toList());
		target.setConfigurationInfos(configurations);
		if (configurations != null) {
			target.setStatusSummaryMap(configurations.stream().peek(i -> {
				if (i.getStatus() == null) {
				...
				if (config.getStatus() == null) {
					throw new IllegalStateException("Configuration info " + config.getConfigurationLabel()
							+ " has null status. Check populator of configuration type "
							+ config.getConfiguratorType());
				}
