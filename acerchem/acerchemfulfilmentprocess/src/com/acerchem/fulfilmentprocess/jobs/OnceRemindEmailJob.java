 ...
 * 此为再次提醒邮件Job
 */
public class OnceRemindEmailJob extends AbstractJobPerformable<CronJobModel>
{ private static final Logger LOG = Logger.getLogger(OnceRemindEmailJob.class);
	...
	public static final String ONCE_REMIND_PROCESS = "order-onceRemind-process";
	...
	public OrderProcessModel createProcess(final OrderModel order)
	{
		...
		final OrderProcessModel process = (OrderProcessModel) businessProcessService.createProcess(
				"severRemind" + System.currentTimeMillis()+random.nextInt(1000),ONCE_REMIND_PROCESS);
	...
	public PerformResult perform(CronJobModel cronJobModel) {
		...
		String query = "select {l.PK} from {order AS l} where {l.status} = ?status" +
				" and {l.onceAgainRemindDate} >= ?startTime  and {l.onceAgainRemindDate} < ?endTime";
