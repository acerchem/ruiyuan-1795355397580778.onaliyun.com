 ...
 * 此为初次提醒邮件Job
 */
public class FirstRemindEmailJob extends AbstractJobPerformable<CronJobModel>
{ private static final Logger LOG = Logger.getLogger(FirstRemindEmailJob.class);
	...
	public static final String FIRST_REMIND_PROCESS = "order-firstRemind-process";
	...
	public OrderProcessModel createProcess(final OrderModel order)
	{
		...
		final OrderProcessModel process = (OrderProcessModel) businessProcessService.createProcess(
				"severRemind" + System.currentTimeMillis()+random.nextInt(1000),FIRST_REMIND_PROCESS);
	...
	public PerformResult perform(CronJobModel cronJobModel) {
		...
		String query = "select {l.PK} from {order AS l} where {l.status} = ?status" +
				" and {l.firstTimeRemindDate} >= ?startTime  and {l.firstTimeRemindDate} < ?endTime";
